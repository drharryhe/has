...
type IServer interface {
	...
	RegisterService(service IService, args ...Any)
	RequestService(service string, slot string, params Map) (Any, *herrors.Error)
}

type IService interface {
	Open(s IServer, instance IService, args ...Any) *herrors.Error
	Close()
	Name() string
	...
	//服务调用相关方法
	Request(slot string, params Map) (Any, *herrors.Error)
}
...
type IRouter interface {
	...
	RequestService(service string, slot string, params Map) (Any, *herrors.Error) //同步请求服务

	// 实体治理相关方法
	AllEntities() []*EntityMeta
	RegisterEntity(m IEntity) *herrors.Error
	ManageEntity(mm *EntityMeta, slot string, params Map) (Any, *herrors.Error)
}
...
type IAPIDataPacker interface {
	Open() *herrors.Error
	Close()
	Marshal(data Any) ([]byte, *herrors.Error)
	Unmarshal(bytes []byte) (Any, *herrors.Error)
}

type IAPIMiddleware interface {
	Open(gw IAPIGateway, ins IAPIMiddleware) *herrors.Error
	Close()
	HandleIn(seq uint64, version string, api string, data Map) (stop bool, err *herrors.Error)                      //入口处理
	HandleOut(seq uint64, version string, api string, result Any, e *herrors.Error) (stop bool, err *herrors.Error) //出口处理
	Type() string
}
...
type IAPIGateway interface {
	...
	I18n() IAPIi18n
	RequestAPI(version string, api string, params Map) (Any, *herrors.Error)
}
