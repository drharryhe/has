...
// BeeLogger is default hlogger in beego application.
...
// if the buffering chan is full, hlogger adapters write to file or other way.
...
// SetLogger provides a given hlogger adapter into BeeLogger with config string.
// config need to be correct JSON as string: {"interval":360}.
func (bl *BeeLogger) setLogger(adapterName string, configs ...string) error {
	config := append(configs, "{}")[0]
	for _, l := range bl.outputs {
		if l.name == adapterName {
			return fmt.Errorf("logs: duplicate adaptername %q (you have set this hlogger before)", adapterName)
		}
	}
...
// DelLogger remove a hlogger adapter in BeeLogger.
...
// If message level (such as LevelDebug) is higher than hlogger level (such as LevelWarning),
...
// Close close hlogger, flush all chan data and destroy all adapters in BeeLogger.
...
// beeLogger references the used application hlogger.
...
// SetLevel sets the global log level used by the simple hlogger.
func SetLevel(l int) {
